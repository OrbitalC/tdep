...
use gottochblandat, only: open_file, lo_invert3x3matrix, lo_chop, lo_determ, lo_sqnorm, &
                          lo_return_unique, lo_clean_fractional_coordinates
...
subroutine commensurate_qpoints(proj)
    ...
    generate_qpoints: block
        ...
        ! We need to reconstruct the supercell reciprocal matrix
        m0 = lo_invert3x3matrix(real(proj%supercellmatrix, r8))
        reclat = matmul(m0, proj%uc%inv_latticevectors)
        reclat = lo_chop(reclat, lo_rectol)
        ...
        if (proj%nqpt .lt. abs(lo_determ(proj%supercellmatrix))) then
            call lo_stop_gracefully(['Could not map all commensurate q-points of the supercell'], lo_exitcode_baddim, &
                                    __FILE__, __LINE__)
        end if
